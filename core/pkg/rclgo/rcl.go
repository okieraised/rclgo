...
// Args ROS2 is configured via CLI arguments, so merge them from different sources.
...
func (c *Context) NewNode(nodeName, namespace string) (node *Node, err error) {
	node = &Node{
		rclNodeT: (*C.rcl_node_t)(C.malloc(C.sizeof_rcl_node_t)),
		context:  c,
	}
	*node.rclNodeT = C.rcl_get_zero_initialized_node()
	...
	rclNodeOptions := C.rcl_node_get_default_options()
	rc := C.rcl_node_init(
		node.rclNodeT,
		cname,
		cNamespace,
	...
	cStr := C.rcl_node_get_name(node.rclNodeT)
	if cStr == nil {
		return nil, errors.New("unexpectedly invalid node")
	}
	node.name = C.GoString(cStr)
	cStr = C.rcl_node_get_namespace(node.rclNodeT)
	if cStr == nil {
		return nil, errors.New("unexpectedly invalid node")
	}
	node.namespace = C.GoString(cStr)
	cStr = C.rcl_node_get_fully_qualified_name(node.rclNodeT)
	if cStr == nil {
		return nil, errors.New("unexpectedly invalid node")
	}
	node.fullyQualifiedName = C.GoString(cStr)
	loggerName := C.rcl_node_get_logger_name(node.rclNodeT)
	if loggerName == nil {
		return nil, errors.New("unexpectedly invalid node")
	}
...
func (n *Node) Close() error {
	...
	rc := C.rcl_node_fini(n.rclNodeT)
	if rc != C.RCL_RET_OK {
		err = errors.Join(err, errorsCast(rc))
	}
	C.free(unsafe.Pointer(n.rclNodeT))
	n.rclNodeT = nil
...
func (n *Node) NewPublisher(
	...
	pub = &Publisher{
		...
		typeSupport:   ros2msg,
		node:          n,
		rclPublisherT: (*C.rcl_publisher_t)(C.malloc(C.sizeof_rcl_publisher_t)),
		topicName:     C.CString(topicName),
	}
	*pub.rclPublisherT = C.rcl_get_zero_initialized_publisher()
	defer onErr(&err, pub.Close)
	rclPublisherOptionsT := C.rcl_publisher_get_default_options()
	rclPublisherOptionsT.allocator = *n.context.rclAllocatorT
	options.Qos.asCStruct(&rclPublisherOptionsT.qos)
...
func (p *Publisher) Publish(ros2msg types.Message) error {
	...
	rc = C.rcl_publish(p.rclPublisherT, ptr, nil)
	if rc != C.RCL_RET_OK {
...
func (p *Publisher) PublishSerialized(msg []byte) error {
	...
	rc := C.rcl_publish_serialized_message(p.rclPublisherT, rclMsg.c(), nil)
	if rc != C.RCL_RET_OK {
...
func (p *Publisher) GetSubscriptionCount() (int, error) {
	var count C.size_t
	rc := C.rcl_publisher_get_subscription_count(p.rclPublisherT, &count)
	if rc != C.RCL_RET_OK {
...
func (p *Publisher) Close() (err error) {
	if p.rclPublisherT == nil {
		return closeErr("publisher")
	}
	p.node.removeResource(p)
	rc := C.rcl_publisher_fini(p.rclPublisherT, p.node.rclNodeT)
	if rc != C.RCL_RET_OK {
		err = errors.Join(err, errorsCast(rc))
	}
	C.free(unsafe.Pointer(p.rclPublisherT))
	p.rclPublisherT = nil
...
func (c *Context) NewClock(clockType ClockType) (clock *Clock, err error) {
	...
	clock.rclClockT = (*C.rcl_clock_t)(C.calloc(1, C.sizeof_rcl_clock_t))
...
func (c *Clock) Close() (err error) {
	...
	rc := C.rcl_clock_fini(c.rclClockT)
	if rc != C.RCL_RET_OK {
		err = errors.Join(err, errorsCast(rc))
	}
	C.free(unsafe.Pointer(c.rclClockT))
	c.rclClockT = nil
	return err
}
...
func (c *Clock) now() (time.Duration, error) {
	...
	rc := C.rcl_clock_get_now(c.rclClockT, &t)
	if rc != C.RCL_RET_OK {
...
func (c *Context) NewTimer(timeout time.Duration, timerCallback func(*Timer)) (timer *Timer, err error) {
	...
	timer = &Timer{
		rclTimerT: (*C.rcl_timer_t)(C.malloc(C.sizeof_rcl_timer_t)),
...
func (t *Timer) GetTimeUntilNextCall() (int64, error) {
	var timeUntilNextCall C.int64_t
	rc := C.rcl_timer_get_time_until_next_call(t.rclTimerT, &timeUntilNextCall)
	if rc != C.RCL_RET_OK {
		return 0, errorsCast(rc)
	}
	return int64(timeUntilNextCall), nil
}

func (t *Timer) Reset() error {
	rc := C.rcl_timer_reset(t.rclTimerT)
	if rc != C.RCL_RET_OK {
...
func (t *Timer) Close() (err error) {
	...
	rc := C.rcl_timer_fini(t.rclTimerT)
	if rc != C.RCL_RET_OK {
		err = errors.Join(err, errorsCast(rc))
	}
	C.free(unsafe.Pointer(t.rclTimerT))
	t.rclTimerT = nil
	return err
}
...
func (n *Node) NewSubscription(
	...
	sub = &Subscription{
		...
		Callback:         subscriptionCallback,
		node:             n,
		rclSubscriptionT: (*C.rcl_subscription_t)(C.malloc(C.sizeof_rcl_subscription_t)),
		topicName:        C.CString(topicName),
	}
	*sub.rclSubscriptionT = C.rcl_get_zero_initialized_subscription()
	defer onErr(&err, sub.Close)
	rclOpts := C.rcl_subscription_get_default_options()
	rclOpts.allocator = *n.context.rclAllocatorT
	options.Qos.asCStruct(&rclOpts.qos)

	rc := C.rcl_subscription_init(
		sub.rclSubscriptionT,
		n.rclNodeT,
		(*C.rosidl_message_type_support_t)(ros2msg.TypeSupport()),
		sub.topicName,
		&rclOpts,
...
func (s *Subscription) TakeMessage(out types.Message) (*MessageInfo, error) {
	...
	defer s.Ros2MsgType.ReleaseMemory(ros2MsgReceiveBuffer)

	rc := C.rcl_take(s.rclSubscriptionT, ros2MsgReceiveBuffer, &rmwMessageInfo, nil)
	if rc != C.RCL_RET_OK {
		return nil, errorsCastC(rc, fmt.Sprintf("rcl_take() failed for subscription='%+v'", s))
	}
	s.Ros2MsgType.AsGoStruct(out, ros2MsgReceiveBuffer)
	return &MessageInfo{
		SourceTimestamp:   time.Unix(0, int64(rmwMessageInfo.source_timestamp)),
		ReceivedTimestamp: time.Unix(0, int64(rmwMessageInfo.received_timestamp)),
		FromIntraProcess:  bool(rmwMessageInfo.from_intra_process),
	}, nil
}
...
func (s *Subscription) TakeSerializedMessage() ([]byte, *MessageInfo, error) {
	...
	defer msg.Close()
	rc := C.rcl_take_serialized_message(s.rclSubscriptionT, msg.c(), &info, nil)
	if rc != C.RCL_RET_OK {
...
func (s *Subscription) GetPublisherCount() (int, error) {
	var count C.size_t
	rc := C.rcl_subscription_get_publisher_count(s.rclSubscriptionT, &count)
	if rc != C.RCL_RET_OK {
...
func (s *Subscription) Close() (err error) {
	if s.rclSubscriptionT == nil {
		return closeErr("subscription")
	}
	s.node.removeResource(s)
	rc := C.rcl_subscription_fini(s.rclSubscriptionT, s.node.rclNodeT)
	if rc != C.RCL_RET_OK {
		err = errors.Join(err, errorsCast(rc))
	}
	C.free(unsafe.Pointer(s.rclSubscriptionT))
	s.rclSubscriptionT = nil
...
func (n *Node) NewService(
	...
	*s.rclService = C.rcl_get_zero_initialized_service()
	...
	opts := C.rcl_service_options_t{allocator: *n.context.rclAllocatorT}
...
func (s *Service) Close() (err error) {
	...
	s.node.removeResource(s)
	rc := C.rcl_service_fini(s.rclService, s.node.rclNodeT)
	if rc != C.RCL_RET_OK {
...
func (s *Service) handleRequest() {
	...
	default:
		_ = s.node.Logger().Debug(errorsCastC(rc, "failed to take request"))
	}
}
...
func (n *Node) NewClient(
	...
	*c.rclClient = C.rcl_get_zero_initialized_client()
	defer onErr(&err, c.Close)
	opts := C.rcl_client_options_t{allocator: *n.context.rclAllocatorT}
	options.Qos.asCStruct(&opts.qos)
...
func (c *Client) Close() error {
	...
	err := c.sender.Close()
	rc := C.rcl_client_fini(c.rclClient, c.node.rclNodeT)
	if rc != C.RCL_RET_OK {
	...
	respChan, otherData := func() (chan *sendResult, interface{}) {
		...
		if err != nil {
			if !errors.Is(err, errTakeFailed) {
				_ = s.transport.Logger.Error(err)
			}
			return nil, nil
		}
