...
type goalRequestMessage interface {
	goalIDMessage
	GetGoalDescription() Message
	SetGoalDescription(goal Message)
}
...
func (g *GoalHandle) setState(event C.rcl_action_goal_event_t) {
	...
	if rc == C.RCL_RET_OK {
		...
		_ = g.server.node.Logger().Debug(errorsCastC(rc, "failed to update goal state"))
	}
}
...
func (g *GoalHandle) Accept() (s *FeedbackSender, err error) {
	...
	if g.status() == GoalUnknown {
		g.info.goal_id.uuid = *(*[GoalIDLen]C.uchar)(unsafe.Pointer(&g.ID))
...
func (g *GoalHandle) setResult(msg Message) {
	g.resultCond.L.Lock()
	defer g.resultCond.L.Unlock()
...
func (g *GoalHandle) waitResult() (GoalStatus, Message) {
	g.resultCond.L.Lock()
	for {
		status := g.status()
		switch status {
...
func (a *action) TypeSupport() ActionTypeSupport {
...
func (n *Node) NewActionServer(
	...
	s := &ActionServer{
		...
		goals: make(map[GoalID]*GoalHandle),
	}
	if s.clock == nil {
	...
	rclOpts := C.rcl_action_server_options_t{
		allocator: *n.context.rclAllocatorT,
...
func (s *ActionServer) Close() (err error) {
	...
	rc := C.rcl_action_server_fini(&s.rclServer, s.node.rclNodeT)
	if rc != C.RCL_RET_OK {
...
func (s *ActionServer) expireGoals() error {
	...
	for {
		...
		for _, goal := range expiredGoals[:int(numExpired)] {
			delete(s.goals, *(*GoalID)(unsafe.Pointer(&goal.goal_id.uuid)))
		}
	}
}
...
func (s *ActionServer) getGoal(id *GoalID, takeLock bool) *GoalHandle {
	if takeLock {
		s.goalsMu.RLock()
...
func (s *ActionServer) sendResultResponse(status GoalStatus, result Message, header *C.rmw_request_id_t) error {
	...
	go func() {
		...
		if rc != C.RCL_RET_OK {
			if rc != C.RCL_RET_ACTION_SERVER_TAKE_FAILED {
				_ = s.node.Logger().Error(errorsCastC(rc, "failed to take cancel request"))
			}
			return
		}
		err := s.processCancelRequest(reqBuf, &respBuf)
		if err != nil {
			_ = s.node.Logger().Error(err)
			return
		}
		...
		func() {
			...
			resp.(forEach).CallForEach(func(toCancel interface{}) {
				goal := s.getGoal(toCancel.(*GoalID), false)
				if goal != nil {
					goal.startCancel()
				}
			})
		...
		if rc != C.RCL_RET_OK {
			_ = s.node.Logger().Error(errorsCastC(rc, "failed to send cancel response"))
		}
	}()
}

func (s *ActionServer) sendFeedback(goal *GoalHandle, fb Message) error {
...
func (s *ActionServer) sendStatuses(lock bool) {
	...
	if rc != C.RCL_RET_OK {
		_ = s.node.Logger().Error("failed to get status array: ", errorsCast(rc))
		return
	}
	rc = C.rcl_action_publish_status(&s.rclServer, unsafe.Pointer(&statuses.msg))
	if rc != C.RCL_RET_OK {
		_ = s.node.Logger().Error("failed to publish status array: ", errorsCast(rc))
	}
}
...
func (s *ActionServer) handleReadyEntities(ctx context.Context, ws *WaitSet) {
	...
	s.rclServerMu.Unlock()
	if rc != C.RCL_RET_OK {
		_ = s.node.Logger().Error(errorsCastC(rc, "failed to get ready entities"))
		return
	}
	if goalReq {
...
type actionClientHandler = func(context.Context, Message)
...
func (m actionClientHandlerMap) call(msg Message) {
	for _, entry := range m {
		entry := entry
...
func (c *ActionClient) Close() error {
	...
	rc := C.rcl_action_client_fini(&c.rclClient, c.node.rclNodeT)
	if rc != C.RCL_RET_OK {
		err = errors.Join(
			err,
...
// ActionTypeSupport.FeedbackMessage().
...
// ActionTypeSupport.SendGoal().Response().
...
func (c *ActionClient) newSendGoalRequest(goal Message) (goalRequestMessage, error) {
...
//
// The type support of the returned message is
// ActionTypeSupport.GetResult().Response().
...
func (c *ActionClient) handleStatus() {
	...
	case C.RCL_RET_ACTION_CLIENT_TAKE_FAILED:
	default:
		_ = c.node.Logger().Error(errorsCastC(rc, "failed to take status"))
	}
}
